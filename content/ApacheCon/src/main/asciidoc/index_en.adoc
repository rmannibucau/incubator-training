...
link:https://twitter.com/rmannibucau[image:romain/twitter.png[width=80%], window="_blank"]
link:https://rmannibucau.metawerx.net[image:romain/blog.png[width=80%], window="_blank"]
link:https://github.com/rmannibucau[image:romain/github.png[width=80%], window="_blank"]
link:https://www.yupiik.com/[image:romain/yupiik.png[width=80%], window="_blank"]


== Agenda

- Why HerdDB ?
- Advantages and challenges for an embedded SQL Database
- HerdDB model and how replication works
- Benchmarks with YCSB
- Introducing JPA - Java Persistence API
- JPA Challenges
- OpenJPA meets HerdDB
- Challenges switching from H2 to HerdDB
- HerdDB on Kubernetes

== Introducing HerdDB

image:herddblogo.png[HerdDB]
...
* EmailSuccess is written in Java and it uses a DB to store message state.
...
[%step]
* As SQL DB for EmailSuccess.com standalone and cluster
* As Database for Apache Pulsar Manager (https://github.com/apache/pulsar-manager)
* As Metadata Service on BlobIt – Binary Large Object Storage (BK based) (https://github.com/diennea/blobit)
* As Configuration and Certificate store for CarapaceProxy server (https://github.com/diennea/carapaceproxy)
...
== Replication: high-availability and scalability: challenges
...
* Understand storage topology: WAL (BK) and Data have separate replication mechanisms.
...
* With one DataSource/JDBC URL you can connect to multiple tablespaces and servers
* Table: table is a table !
* Transactions: Transactions must span tables of the same TableSpace
* Joins: Queries can only refer to tables of the same TableSpace
...
[%step]
* Data Page: a bunch of records, indexed by PK (hashmap)
...
[.smaller]
NOTE: result captured with HerdDB 0.19 and MySQL 5.7.24 on a bare metal machine with SSD, on Linux, JDK13.
...
+
> OpenJPA OOTB dictionaries are case insensitive for example.
...
[source,java]
----
public class HerdDBDictionary extends DBDictionary {
...
** StatefulSet (combine deployments and PV/PVC) - better but has some "bugs"
...
* Enables to deploy the same application on premise or cloud providers
